...
class BDD100kDatasetLabel(data.Dataset): #return target(BDD100K) dataset with corresponding labels
    # for reading BDD100K with labels
    ...
    def __getitem__(self, index):
        ...
        image = Image.open( osp.join(self.root, "images/clear/%s/%s" % (self.set, name)) ).convert('RGB')
        lbname = name.replace(".jpg", "_train_id.png")
        label = Image.open(osp.join(   self.root, "labels/clear/%s/%s" % (self.set, lbname)   ))
